...
const NAV = {
  super_admin:  [['dashboard','Dashboard','home'],['orgs','Organisations','users'],['users','Users','users'],['audit','Audit Log','audit'],['teams','Teams','users'],['support','Support','alert'],['notifications','Announcements','alert']],
...
const [existingUserMsg, setExistingUserMsg] = useState('')
  ...
  return (
    <div className="anim">
          ...
          {filtered.map(org=>(
            <div key={org.id} style={{background:'var(--s2)',border:'1px solid var(--border)',borderRadius:10,padding:'12px 14px',borderLeft:'4px solid '+(org.status==='active'?'var(--green)':'var(--border)')}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',marginBottom:5}}>
                ...
                <span style={{fontWeight:700,fontSize:14,cursor:'pointer',color:'var(--brand)',textDecoration:'underline',flex:'1 1 120px',minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}} onClick={()=>{ setSelectedOrgView(org); loadOrgMembers(org.name) }}>{org.name}</span>
      ...
      {/* View Member Profile Modal (now only in org context early return) */}
      {viewingMember&&(
        <div className="modal-overlay" onClick={()=>{setViewingMember(null);setShowMemberOrgChange(false);setMemberOrgSearch('')}}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-hdr"><div className="modal-title">👤 Member Profile</div><button className="modal-close" onClick={()=>{setViewingMember(null);setShowMemberOrgChange(false);setMemberOrgSearch('')}}>×</button></div>
