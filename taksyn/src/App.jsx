...
function ReportsView({ tasks, user }) {
  ...
  const exportCompliancePDF = () => {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Taksyn Compliance Report</title><style>${baseStyle}.sg{grid-template-columns:repeat(5,1fr)}</style></head><body>${reportHeader('Compliance Report')}<div class="sg"><div class="st"><div class="sv">${total}</div><div class="sl">Total Tasks</div></div><div class="st"><div class="sv g">${done}</div><div class="sl">Completed</div></div><div class="st"><div class="sv a">${total-done}</div><div class="sl">Not Completed</div></div><div class="st"><div class="sv r">${overdue}</div><div class="sl">Overdue</div></div><div class="st"><div class="sv a">${totalToReview}</div><div class="sl">Total for Review</div></div><div class="st"><div class="sv a">${pendingReview}</div><div class="sl">Pending Review</div></div><div class="st"><div class="sv g">${reviewed}</div><div class="sl">Reviewed</div></div><div class="st"><div class="sv g">${approved}</div><div class="sl">Approved</div></div><div class="st"><div class="sv r">${rejected}</div><div class="sl">Rejected</div></div><div class="st"><div class="sv g">${doneOnDayPct}%</div><div class="sl">Tasks Done Same Day</div></div><div class="st"><div class="sv g">${reviewedInTimePct}%</div><div class="sl">Reviewed in Time</div></div><div class="st"><div class="sv g">${reportWithinWeekPct}%</div><div class="sl">Report Reviewed in Time</div></div><div class="st"><div class="sv" style="color:#8B5CF6">${pct(compDone,compT.length)}%</div><div class="sl">Compliance Rate</div></div></div><table><thead><tr><th>ID</th><th>Task</th><th>Category</th><th>Status</th><th>Compliance</th><th>Due Date</th><th>Completed</th><th>Duration</th><th>Assigned To</th></tr></thead><tbody>${rows}</tbody></table>${reportFooter}</body></html>`
    openReport(html)
  }
        ...
        <>
          <div className="section">
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(110px,1fr))',gap:10,marginTop:10}}>
              {[['Total',total,''],['Completed',done,'g'],['Not Completed',total-done,'a'],['Overdue',overdue,'r'],['Total for Review',totalToReview,'a'],['Pending Review',pendingReview,'a'],['Reviewed',reviewed,'g'],['Approved',approved,'g'],['Rejected',rejected,'r'],['Tasks Done Same Day',doneOnDayPct+'%','g'],['Reviewed in Time',reviewedInTimePct+'%','g'],['Report Reviewed in Time',reportWithinWeekPct+'%','g'],['Compliance Rate',pct(compDone,compT.length)+'%','p']].map(([l,v,c])=>(
