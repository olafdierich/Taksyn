...
const GUIDE_CONTENT = {
  ...
  manager: {
    ...
    chapters: [
      ...
      { num:4, title:'Managing Tasks (Supervisor & Manager)', subChapters:[
        ...
        { id:'4.2', title:'Reviewing submitted tasks', steps:[
          ...
          { summary:'Open the task and review the checklist, photos and timestamps', detail:'Click on a task to open its detail view. Check that all required checklist items are ticked, review the photo evidence to confirm the work was done correctly, and check the timestamps to verify the task was completed at the right time.', foundIn:'Tasks → task detail → Checklist section, Evidence section, timestamps', pitfall:'Never approve a task with missing photos if photo evidence is required.' },
          { summary:'Click Approve to complete or Reject to send back to the worker with comments', detail:'If the work meets the required standard click Approve — the task is marked as completed. If the work is not acceptable click Reject, type a clear reason in the rejection comment box, and submit. The worker receives your comment and the task returns to their queue for rework.', foundIn:'Task detail → Approve button or Reject button → rejection comment box', tip:'Always add a comment when rejecting so the worker knows what to fix.' },
        ]},
        { id:'4.3', title:'Finding tasks in the Archive', steps:[
          { summary:'Go to Tasks and click Archive', detail:'The Archive contains all completed, approved, and old tasks. Click Tasks in the left sidebar then click the Archive tab to access it.', foundIn:'Left sidebar → Tasks → Archive tab', tip:'Check the archive regularly to monitor compliance history.' },
          { summary:'Search or filter by date, worker or status to find completed or deleted tasks', detail:'Use the filter and search tools within the Archive to locate specific tasks. You can filter by date range, assigned worker, or task status to quickly find what you need.', foundIn:'Tasks → Archive tab → filter bar', tip:'Check the archive regularly to monitor compliance history.' },
        ]},
        { id:'4.4', title:'Editing and deleting tasks', steps:[
          { summary:'Open a task and click Edit to change details, or Delete to remove it', detail:'Open any active task to see the Edit and Delete options. Click Edit to modify the task title, description, checklist, due date, priority, or assigned worker. Click Delete to permanently remove the task from the active list.', foundIn:'Tasks → task detail → Edit button, Delete button', pitfall:'Deletion cannot be undone — use escalation instead if you want to flag a task without removing it.' },
          { summary:'Deleted tasks are permanently removed from the active list but remain in the Audit Log for compliance', detail:'When you delete a task it is removed from the active task list immediately. However a record of the deletion is kept in the Audit Log, including who deleted it and when, ensuring there is always a compliance trail.', foundIn:'Tasks → task detail → Delete button; Audit Log for deletion records', pitfall:'Deletion cannot be undone — use escalation instead if you want to flag a task without removing it.' },
        ]},
        { id:'4.5', title:'Escalating a task', steps:[
          { summary:'Open a task and click Escalate to flag it as urgent', detail:'If a task requires immediate attention from a higher level, open the task detail view and click the Escalate button. Add a brief note explaining why you are escalating.', foundIn:'Tasks → task detail → Escalate button', tip:'Escalate early rather than late — do not wait until a deadline is missed.' },
          { summary:'The task moves to the Escalations view and notifies the client admin', detail:'Once escalated the task appears in the Escalations view and the Client Admin receives a notification. They can then review the situation and take action at a higher level.', foundIn:'Escalations view; Client Admin notification', tip:'Escalate early rather than late — do not wait until a deadline is missed.' },
        ]},
      ]},
    ]
  },
