            ...
            <div className="modal-body">
              ...
              <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
                ...
                <button className="btn btn-danger" disabled={!rejectNote.trim()} onClick={()=>{
                  ...
                  update(showReject,{status:'rejected',comments:[...(task.comments||[]),'⚠️ '+user.name+' (Supervisor): '+rejectNote.trim()]})
...
function EvidenceView({ tasks, setTasks, user }) {
  ...
  const approve = async (id) => { setTasks(prev=>prev.map(t=>t.id===id?{...t,status:'approved'}:t)); if(isConfigured()) await supabase.from('tasks').update({status:'approved'}).eq('id',id) }
  const reject = async (id) => { setTasks(prev=>prev.map(t=>t.id===id?{...t,status:'rejected'}:t)); if(isConfigured()) await supabase.from('tasks').update({status:'rejected'}).eq('id',id) }
  return (
    <div className="anim">
...
function ReportsView({ tasks, user }) {
  ...
  const exportPDF = () => {
    ...
    const html='<!DOCTYPE html><html><head><meta charset="utf-8"><title>Taksyn Compliance Report</title><style>*{box-sizing:border-box}body{font-family:Helvetica Neue,sans-serif;padding:40px;color:#1a2033;font-size:13px}.hdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;padding-bottom:16px;border-bottom:2px solid #00A87E}.lt{font-size:24px;font-weight:800;color:#00A87E}.ri{text-align:right;font-size:11px;color:#5a6478}.ri strong{display:block;font-size:14px;color:#1a2033;margin-bottom:2px}.sg{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px}.st{background:#f4f6f9;border-radius:8px;padding:14px;text-align:center}.sv{font-size:22px;font-weight:800;color:#00A87E;line-height:1}.sv.r{color:#EF4444}.sv.g{color:#10B981}.sv.a{color:#F59E0B}.sl{font-size:10px;color:#5a6478;margin-top:5px;text-transform:uppercase}table{width:100%;border-collapse:collapse;font-size:11px}th{text-align:left;padding:7px 8px;background:#f4f6f9;font-size:9px;text-transform:uppercase;color:#5a6478;border-bottom:1px solid #e8ebf0}td{padding:7px 8px;border-bottom:1px solid #f0f2f5}.ft{margin-top:28px;padding-top:14px;border-top:1px solid #e8ebf0;font-size:10px;color:#9aa3b2;display:flex;justify-content:space-between}</style></head><body><div class="hdr"><div><img src="https://taksyn.vercel.app/logo.jpeg" height="40" style="object-fit:contain" onerror="this.style.display='none'"/><span class="lt" style="margin-left:10px">Taksyn</span></div><div class="ri"><strong>Compliance Report</strong>'+user.org+'<br/>Period: '+pl+'<br/>Generated: '+new Date().toLocaleDateString('en-AU',{day:'numeric',month:'long',year:'numeric'})+'</div></div><div class="sg"><div class="st"><div class="sv">'+total+'</div><div class="sl">Total Tasks</div></div><div class="st"><div class="sv g">'+done+'</div><div class="sl">Completed</div></div><div class="st"><div class="sv a">'+(total-done)+'</div><div class="sl">Not Completed</div></div><div class="st"><div class="sv r">'+overdue+'</div><div class="sl">Overdue</div></div><div class="st"><div class="sv g">'+approved+'</div><div class="sl">Approved</div></div><div class="st"><div class="sv r">'+rejected+'</div><div class="sl">Rejected</div></div><div class="st"><div class="sv a">'+notOnTime+'</div><div class="sl">Not On Time</div></div><div class="st"><div class="sv" style="color:#8B5CF6">'+pct(compDone,compT.length)+'%</div><div class="sl">Compliance Rate</div></div></div><table><thead><tr><th>ID</th><th>Task</th><th>Category</th><th>Status</th><th>Compliance</th><th>Due Date</th><th>Completed</th><th>Duration</th><th>Assigned To</th></tr></thead><tbody>'+rows+'</tbody></table><div class="ft"><span>Taksyn — Task Compliance & Accountability Platform</span><span>taksyn.vercel.app</span></div></body></html>'
