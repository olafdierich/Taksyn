...
const CAN_REMOVE_ROLES = ['client_admin', 'manager']
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[delete-user] received:', { userId, orgId, orgName, mode })
    ...
    const { data: callerProfiles } = await supabaseAdmin
      .from('profiles').select('role').eq('id', caller.id)
    const isSuperAdmin = (callerProfiles || []).some((r: { role?: string }) => r.role === 'super_admin')
    ...
    if (orgId) {
      const { data: tOm } = await supabaseAdmin
        .from('org_members').select('role').eq('user_id', userId).eq('org', orgId).maybeSingle()
    ...
    if (!targetLevel) {
      ...
      const { data: tProfs } = await supabaseAdmin
        .from('profiles').select('role').eq('id', userId)
      targetLevel = (tProfs || []).reduce(
        (max: number, r: { role?: string }) => Math.max(max, ROLE_LEVEL[r.role ?? ''] ?? 0), 0)
    }

    if (mode === 'purge') {
      // Full account wipe is super_admin ONLY.
      if (!isSuperAdmin) {
        return json({ error: 'Only a super admin can permanently delete an account.' }, 403)
      }
    } else {
      ...
      if (!isSuperAdmin) {
        if (!orgId) return json({ error: 'orgId is required for removal.' }, 400)
        const { data: membership } = await supabaseAdmin
          .from('org_members').select('role').eq('user_id', caller.id).eq('org', orgId).maybeSingle()
        ...
        if (targetLevel >= callerLevel) {
          return json({ error: 'Cannot remove a user at or above your own level.' }, 403)
        }
      }
    }
    // ---- END AUTH GATE ----
    ...
    if (mode === 'purge') {
      const { error, count } = await supabaseAdmin
        .from('team_members').delete({ count: 'exact' }).eq('user_id', userId)
      results.team_members = error ? { error: error.message } : { deleted: count }
    } else {
      ...
      const orgMatch: string[] = [orgId, orgName].filter(Boolean) as string[]
      let teamIds: string[] = []
      if (orgMatch.length) {
        const { data: teamRows } = await supabaseAdmin
          .from('teams').select('id').in('org', orgMatch)
        teamIds = (teamRows || []).map((t: { id: string }) => t.id)
      }
      if (teamIds.length) {
        const { error, count } = await supabaseAdmin
          .from('team_members').delete({ count: 'exact' })
          .eq('user_id', userId).in('team_id', teamIds)
        results.team_members = error ? { error: error.message } : { deleted: count }
      } else {
        results.team_members = { deleted: 0, note: 'no teams matched org' }
      }
    }
    ...
    // --- invite_links: by email + org (remove) or by email across all orgs (purge) ---
    {
      ...
      const email = (profRows || []).map((r: { email?: string }) => r.email).find(Boolean) || null
      if (email) {
        let q = supabaseAdmin.from('invite_links').delete({ count: 'exact' })
          .eq('invited_email', email.trim().toLowerCase())
        if (mode !== 'purge' && orgId) q = q.eq('organisation_id', orgId)
        const { error, count } = await q
        results.invite_links = error ? { error: error.message } : { deleted: count }
      } else {
        results.invite_links = { deleted: 0, note: 'no email on profile' }
      }
    }

    // --- tasks: reassign open tasks off this user (org NAME scoped in remove mode) ---
    {
      let q = supabaseAdmin.from('tasks').update({ assigned_user_id: null })
        .eq('assigned_user_id', userId)
        .not('status', 'in', '("completed","approved")')
      if (mode !== 'purge' && orgName) q = q.eq('org', orgName)
      const { error } = await q
      results.tasks_reassigned = error ? { error: error.message } : { ok: true }
    }

    // --- org_members ---
    if (mode === 'purge') {
      const { error, count } = await supabaseAdmin
        .from('org_members').delete({ count: 'exact' }).eq('user_id', userId)
      results.org_members = error ? { error: error.message } : { deleted: count }
    } else {
      if (!orgId) {
        results.org_members = { error: 'orgId required for remove' }
      } else {
        const { error, count } = await supabaseAdmin
          .from('org_members').delete({ count: 'exact' }).eq('user_id', userId).eq('org', orgId)
        results.org_members = error ? { error: error.message } : { deleted: count }
      }
    }
    ...
    if (mode === 'purge') {
      ...
      results.profiles = pErr ? { error: pErr.message } : { deleted: pCount }

      const { error: aErr } = await supabaseAdmin.auth.admin.deleteUser(userId)
      results.auth = aErr ? { error: aErr.message } : { deleted: true }
    } else {
      results.profiles = { skipped: 'remove mode — profile kept' }
      results.auth = { skipped: 'remove mode — login kept' }
    }
