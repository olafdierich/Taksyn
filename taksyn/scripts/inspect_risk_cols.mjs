// READ-ONLY: inspect existing risk columns on sandbox incidents table.
// Reports the current risk_likelihood / risk_consequence / risk_rating values + types
// so the new residual_* columns can mirror them exactly. Writes NOTHING.
...
if (!(data||[]).length) console.log('  (no incident rows in sandbox — types inferred from column existence)');

// Check whether residual_* columns already exist (idempotency for re-runs)
