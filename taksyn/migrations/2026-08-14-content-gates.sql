-- Taksyn migration: content gates on incident_transition.
-- 14 August 2026. A step could be advanced with the work half done.
-- Three gates: findings examined before Action; tasks approved before
-- Review; residual risk rated before Closed. Applied to ALL incidents.
-- Census before applying: only two LIVE records would be blocked, both
-- correctly, and neither is stranded.
-- DERIVED via pg_get_functiondef, not retyped. Prior md5 both envs:
-- 6cd2186ef40470d35e0f713cad778752
...
begin
  ...
  -- ---------------------------------------------------------------------
  -- CONTENT GATES (14 Aug 2026). A step cannot be advanced until the work
  -- it represents is actually present -- not merely until someone presses
  -- the button. Placed AFTER the authority checks: a user who may not act
  -- should be told that, not told their findings are incomplete.
  --
  -- Applied to ALL incidents, with no cutoff date. A gate with a date
  -- exception means two rules running forever and nobody remembering which
  -- record falls under which. Nothing is stranded: the remedy is always to
  -- do the missing work, or mark a finding not applicable.
  -- ---------------------------------------------------------------------
  if v_kind = 'forward' then

    -- Investigation -> Action. not_applicable is a deliberate answer and
    -- passes; not_examined means nobody looked.
    if p_to_status = 'actions_open' then
      if exists (select 1 from public.incident_findings f
                  where f.incident_id = p_incident_id and f.state = 'not_examined') then
        raise exception
          'Every investigation and root cause item must be examined, or marked not applicable, before moving to Action.'
          using errcode = '22023';
      end if;
    end if;

    -- Action -> Review. Task COMPLETION only: effectiveness is judged during
    -- Review, and demanding it beforehand asks whether something worked
    -- before anyone has had the chance to see.
    -- Voided actions are ignored -- they were withdrawn.
      ...
      if exists (select 1 from public.incident_actions a
                  where a.incident_id = p_incident_id
                    ...
                    and (a.task_id is null
                         or not exists (select 1 from public.tasks t
                                         where t.id = a.task_id and t.status = 'approved'))) then
        raise exception
          'Every corrective action must have a completed and approved task before moving to Review.'
          using errcode = '22023';
      end if;
      if not exists (select 1 from public.incident_actions a
                      where a.incident_id = p_incident_id and a.status <> 'void')
         and coalesce(v_inc.no_action_required, false) = false then
        raise exception
          'Record a corrective action, or tick that none is required, before moving to Review.'
          using errcode = '22023';
      end if;
    end if;

    -- Review -> Closed. The closure note is already required above; this
    -- adds the residual rating, which is the whole point of the step.
    if p_to_status = 'closed' then
      if v_inc.residual_rating is null then
        raise exception
          'Rate the residual risk before closing. It is the record of what the corrective actions achieved.'
          using errcode = '22023';
      end if;
    end if;
