...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    console.log('[invite-user] received fields:', {
      action,
      email,
      name,
      role,
    ...
    if (action === 'resend') {
      ...
      if (!isSuperAdmin) {
        const { data: memberships } = await supabaseAdmin.from('org_members').select('role').eq('user_id', caller.id);
      ...
      if (!isSuperAdmin) {
        ...
        const { data: membership } = await supabaseAdmin.from('org_members').select('role').eq('user_id', caller.id).eq('org', orgId).single();
        ...
        if (invitedLevel >= callerLevel) {
          return json({
            error: 'Cannot invite a role at or above your own level'
          }, 403);
        }
      }
    }
